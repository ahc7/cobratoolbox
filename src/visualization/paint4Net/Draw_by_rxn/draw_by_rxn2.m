...
if ~isempty(flux) %if flux vector is not empty
    ...
    if not_present_rxns{1}=='0'; %if all reactions were present in the model 
        ...
        if strcmp(drawMap,'true') %if drawMap=true
            ...
            map = biograph(matrix,ids,'ShowTextInNodes','ID','LayoutType', 'hierarchical',...
                'LayoutScale',0.25,'Scale',0.75,'EdgeFontSize',12) %create a biograph object EdgeType/ 'LayoutType', 'evuilibrium'
