...
function [involvedMets,deadEnds,deadRxns]=draw_by_rxn_pretty(model,rxns,drawMap,direction,initialMet,excludeMets,flux,save,closev)
                ...
                for n=1:length(model.rxns) %cycle through the reactions in the model
                    ...
                    if strcmp(model.rxns{RxIDs(m)},model.rxns{n}) %if particular reaction has been found in the model
                        ...
                        try %try to set label=reaction name
                            set(map.nodes(length(involvedMets)+m), 'Label', num2str(abs(flux(RxIDs(m)))),'Color',[1,1,1],'LineColor',[1,1,1],'Size',[50,10])
                        catch info
                        end
